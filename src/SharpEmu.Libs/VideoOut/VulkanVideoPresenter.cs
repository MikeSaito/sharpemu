...
internal sealed record VulkanGuestMemoryBuffer(
    ulong BaseAddress,
    byte[] Data,
    Func<ulong, ReadOnlySpan<byte>, bool>? PublishGuestMemory = null,
    uint DataFormat = 0,
    uint NumberFormat = 0,
    uint Stride = 0);
...
internal static unsafe class VulkanVideoPresenter
{
    ...
    private static readonly Dictionary<ulong, PublishedBufferFormat> _publishedBufferFormats = new();

    private readonly record struct PublishedBufferFormat(
        uint DataFormat,
        uint NumberFormat,
        uint Stride,
        int Length);

    private static void NotePublishedBufferFormat(
        ulong address,
        uint dataFormat,
        uint numberFormat,
        uint stride,
        int length)
    {
        if (address == 0 || dataFormat == 0 || length <= 0)
        {
            return;
        }
        ...
        lock (_gate)
        {
            _publishedBufferFormats[address] = new PublishedBufferFormat(
                dataFormat,
                numberFormat,
                stride,
                length);
    ...
    internal static bool TryLoadHalfFloatTextureFromPublishedFloatBuffer(
        ...
        uint numberType,
        int halfTexelBytes,
        Func<ulong, byte[], bool> tryReadGuest,
        out byte[] halfPixels)
    {
        halfPixels = [];
        if (address == 0 ||
            halfTexelBytes <= 0 ||
            (halfTexelBytes & 7) != 0 ||
            !IsHalfFloatRgbaTextureFormat(textureFormat, numberType))
        {
            return false;
        }

        PublishedBufferFormat published;
        lock (_gate)
        {
            if (!_publishedBufferFormats.TryGetValue(address, out published))
            {
                return false;
            }
        }
        ...
        var halfTexelCount = halfTexelBytes / 8;
        var floatTexelBytes = checked(halfTexelCount * 16);
        if (floatTexelBytes > published.Length)
        {
            return false;
        }

        var floatPixels = new byte[floatTexelBytes];
        if (!tryReadGuest(address, floatPixels))
        {
            return false;
        }

        halfPixels = new byte[halfTexelBytes];
        for (var texel = 0; texel < halfTexelCount; texel++)
        {
            var floatOffset = texel * 16;
            var halfOffset = texel * 8;
            for (var component = 0; component < 4; component++)
            {
                var bits = BinaryPrimitives.ReadUInt32LittleEndian(
                    floatPixels.AsSpan(floatOffset + component * 4, 4));
                var value = BitConverter.UInt32BitsToSingle(bits);
                BinaryPrimitives.WriteUInt16LittleEndian(
                    halfPixels.AsSpan(halfOffset + component * 2, 2),
    ...
    private static bool IsHalfFloatRgbaTextureFormat(uint textureFormat, uint numberType) =>
        textureFormat == 71 ||
        ((textureFormat is 12 or 22) && numberType == 7);
    ...
    public static bool SubmitComputeDispatch(
        ulong shaderAddress,
        byte[] computeSpirv,
        IReadOnlyList<VulkanGuestDrawTexture> textures,
        IReadOnlyList<VulkanGuestMemoryBuffer> globalMemoryBuffers,
        ...
        uint groupCountZ)
    {
        // Buffer-only CS (e.g. MUBUF STORE_FORMAT fills) have no storage images;
        // Empty().All(...) is vacuously true and used to drop those dispatches.
        if (computeSpirv.Length == 0 ||
            groupCountX == 0 ||
            groupCountY == 0 ||
            groupCountZ == 0 ||
            (globalMemoryBuffers.Count == 0 &&
             textures.All(texture => !texture.IsStorage)))
        {
            return false;
        }
        ...
        uint pitchInPixel)
    {
        ...
        lock (_gate)
        {
            // Completed GPU publish, or a queued/offscreen RT target that has not
            // finished yet. Flip often races the composite offscreen enqueue.
            var known = _gpuGuestImages.ContainsKey(address);
            var queued = _availableGuestImages.ContainsKey(address);
            if (ShouldTracePresentedGuestImageContentsForDiagnostics())
            {
                Console.Error.WriteLine(
                    $"[LOADER][TRACE] vk.submit_call kind=TrySubmitGuestImage addr=0x{address:X16} " +
                    $"{width}x{height} known={known} queued={queued}");
            }
            ...
            _latestPresentation = new Presentation(
                ...
                GuestDrawKind.None,
                TranslatedDraw: null,
                // Wait for in-flight offscreen work (e.g. composite → flip target).
                RequiredGuestWorkSequence: _enqueuedGuestWorkSequence,
                IsSplash: false,
    ...
    private static bool IsKnownGuestTextureFormat(uint format) =>
        format is 4 or 5 or 7 or 9 or 13 or 14 or 22 or 29 or 36 or 56 or 62 or 64 or 71 or 76 or 77;
    ...
    private sealed class Presenter : IDisposable
    {
        ...
        private sealed class GlobalBufferResource
        {
            ...
            public uint Stride;
            public Func<ulong, ReadOnlySpan<byte>, bool>? PublishGuestMemory;
        }
        ...
        private sealed class GuestImageResource
        {
            ...
            public ulong CpuContentFingerprint;
            // Host-tracked layout so sampled draws can re-barrier without guessing.
        ...
        // Guest draws are separate VkQueueSubmit batches. Without a wait, a later
        // draw can record/sample an RT while an earlier submit is still writing it
        // (e.g. upsample 054127 then composite 050741) and see cleared/undefined texels.
        private void DrainInFlightGuestSubmissions()
        {
        ...
        private void CollectCompletedGuestSubmissions(bool waitForOldest)
        {
            ...
            while (_pendingGuestSubmissions.TryPeek(out var submission))
            {
                ...
                WriteBackGlobalMemoryBuffers(submission.Resources);
            ...
            out uint bytesPerPixel)
        {
            ...
            try
            {
                ...
                var oldLayout = image.Layout == ImageLayout.Undefined
        ...
        private TranslatedDrawResources CreatePresentExposureDrawResources(
            GuestImageResource source)
        {
            if (!TryGetExposedCopyFragmentShader(out var fragmentSpirv))
            {
                throw new InvalidOperationException(
                    "present exposure fragment shader unavailable");
            }

            var guestFormat = GetGuestTextureFormat(source.Format);
            if (guestFormat == 0)
            {
                throw new InvalidOperationException(
                    $"unsupported present source format {source.Format}");
            }

            var draw = new VulkanTranslatedGuestDraw(
                VertexSpirv: [],
                PixelSpirv: fragmentSpirv,
                Textures:
                [
                    new VulkanGuestDrawTexture(
                        source.Address,
                        source.Width,
                        source.Height,
                        // Request float sampling so A2R10/B10G11 packs go through
                        // the typed-alias blit into R16F before ImageSample+exposure.
                        Format: 71,
                        NumberType: 0,
                        [],
                        IsFallback: false,
                ...
                IndexBuffer: null,
                RenderState: VulkanGuestRenderState.Default);
            var resources = CreateTranslatedDrawResources(
                draw,
                _renderPass,
                _swapchainFormat,
                _extent);
            resources.DebugName =
                $"SharpEmu present exposure x{GetPresentExposure():0.##}";
            if (resources.Textures is [{ } presentTexture])
            {
                Console.Error.WriteLine(
                    $"[LOADER][TRACE] vk.present_exposure_tex " +
                    $"convert={presentTexture.NeedsGpuConvert} " +
                    $"alias={(presentTexture.AliasSourceImage is not null)} " +
                    $"size={presentTexture.Width}x{presentTexture.Height}");
            }
        ...
        private TextureResource ResolveTextureResource(VulkanGuestDrawTexture texture)
        {
                ...
                IsCompatibleGuestImageAlias(texture, guestImage))
            {
                        ...
                        out var view))
                {
                    ...
                    else if (_tracedTextureCacheHits.Add(
        ...
        {
            resource = default!;
            // Keep HDR/near-black UNORM packs in a float temp before sampling:
            // - fmt36/R8Unorm over B10G11 bloom (existing)
            // - present exposure sampling A2R10 (direct ImageSample of A2R10
            //   returns RGB≈0 on the present path; blit→R16F preserves luminance)
            var sourceIsHdrPack =
                guestImage.Format is Format.B10G11R11UfloatPack32 or Format.A2R10G10B10UnormPack32;
            var requestNeedsFloatAlias =
                requestedFormat is Format.R8Unorm or Format.R16G16B16A16Sfloat;
            var convertFromHdrPack = sourceIsHdrPack && requestNeedsFloatAlias;
            if (!convertFromHdrPack ||
            ...
            var convertFormat = Format.R16G16B16A16Sfloat;
            ...
            var debugName = $"{TextureDebugName(texture, convertFormat)} typed-alias";
            ...
            TraceVulkanShader(
                ...
                $"size={width}x{height} requested={requestedFormat} " +
                $"convert={convertFormat} source={guestImage.Format}");
            ...
            out TextureResource resource)
        {
            ...
            var pixels = texture.ReloadPixels?.Invoke() ?? texture.RgbaPixels;
            if (pixels.Length != (int)expectedSize)
            {
                pixels = texture.RgbaPixels.Length == (int)expectedSize
                    ? texture.RgbaPixels
        ...
        private TextureResource CreateTextureResource(VulkanGuestDrawTexture texture)
        {
            ...
            var sourcePixels = texture.ReloadPixels?.Invoke() ?? texture.RgbaPixels;
            if (texture.Address != 0 &&
                sourcePixels.Length > 0 &&
                _tracedTextureUploads.Contains((texture.Address, width, height, vkFormat)))
            {
                var reloadNonzero = 0;
                var limit = Math.Min(sourcePixels.Length, 64);
                ...
                {
                    TraceVulkanShader(
                        $"vk.texture_reload addr=0x{texture.Address:X16} " +
                        $"bytes={sourcePixels.Length} nonzero64={reloadNonzero} " +
                        $"head={Convert.ToHexString(sourcePixels.AsSpan(0, Math.Min(sourcePixels.Length, 16)))}");
                }
            }
            var pixels = sourcePixels.Length == (int)expectedSize
                ? sourcePixels
            ...
            VulkanGuestMemoryBuffer guestBuffer)
        {
            ...
            return new GlobalBufferResource
            {
                ...
                DataLength = guestBuffer.Data.Length,
                DataFormat = guestBuffer.DataFormat,
                NumberFormat = guestBuffer.NumberFormat,
                Stride = guestBuffer.Stride,
                PublishGuestMemory = guestBuffer.PublishGuestMemory,
            };
        }

        private void WriteBackGlobalMemoryBuffers(TranslatedDrawResources resources)
        {
            foreach (var globalBuffer in resources.GlobalMemoryBuffers)
            {
                if (globalBuffer is null ||
                    globalBuffer.PublishGuestMemory is null ||
                    globalBuffer.BaseAddress == 0 ||
                    globalBuffer.DataLength <= 0 ||
                    globalBuffer.Memory.Handle == 0)
                {
                    continue;
                }

                var byteCount = (ulong)globalBuffer.DataLength;
                void* mapped;
                Check(
                    _vk.MapMemory(_device, globalBuffer.Memory, 0, byteCount, 0, &mapped),
                    "vkMapMemory(global writeback)");
                try
                {
                    var span = new ReadOnlySpan<byte>(mapped, globalBuffer.DataLength);
                    if (!globalBuffer.PublishGuestMemory(globalBuffer.BaseAddress, span))
                    {
                        TraceVulkanShader(
                            $"vk.global_buffer_writeback_failed base=0x{globalBuffer.BaseAddress:X16} " +
                            $"bytes={globalBuffer.DataLength}");
                        continue;
                    }

                    NotePublishedBufferFormat(
                        globalBuffer.BaseAddress,
                        globalBuffer.DataFormat,
                        globalBuffer.NumberFormat,
                        globalBuffer.Stride,
                        globalBuffer.DataLength);
                    ...
                    var head = Math.Min(span.Length, 16);
                    var headHex = Convert.ToHexString(span[..head]);
                    if (_tracedGlobalBuffers.Add((globalBuffer.BaseAddress | 1UL, nonzero)))
                    {
                        TraceVulkanShader(
                            $"vk.global_buffer_writeback base=0x{globalBuffer.BaseAddress:X16} " +
                            $"bytes={globalBuffer.DataLength} nonzero={nonzero} head={headHex}");
                    }

                    InvalidateCpuBackedGuestImages(
                        globalBuffer.BaseAddress,
                        globalBuffer.DataLength);
                }
                finally
                {
                    _vk.UnmapMemory(_device, globalBuffer.Memory);
                }
            }
        }

        private void InvalidateCpuBackedGuestImages(ulong baseAddress, int byteCount)
        {
            if (byteCount <= 0)
            {
                return;
            ...
            var rangeEnd = baseAddress + (ulong)byteCount;
            lock (_gate)
            {
                foreach (var guestImage in _guestImages.Values)
                {
                    if (!guestImage.IsCpuBacked ||
                        guestImage.Address < baseAddress ||
                        guestImage.Address >= rangeEnd)
                    {
                        continue;
                    }

                    guestImage.CpuContentFingerprint = 0;
                    guestImage.Initialized = false;
                    guestImage.InitialUploadPending = false;
                    _gpuGuestImages.Remove(guestImage.Address);
                }
            }
        }
        ...
        private static Format GetTextureFormat(uint format, uint numberType) =>
            (format, numberType) switch
            {
                ...
                (76, _) => Format.R32G32B32A32Sint,
                (77, _) => Format.R32G32B32A32Sfloat,
        ...
        private static Format GetRenderTargetFormat(uint format, uint numberType) =>
            (format, numberType) switch
            {
                ...
                (5, 7) => Format.R16G16Sfloat,
                (6, 7) => Format.B10G11R11UfloatPack32,
                // Present exposure needs sub-10-bit HDR luminance. A2R10 packs
                // R≈1e-3 to 0; keep a float attachment when exposure is active.
                // Use R32F (not R16F): observed R16F display targets filled with
                // half-NaN garbage (0000FF7FFF7F003C) on the present readback path.
        ...
        private void ExecuteOffscreenDraw(VulkanOffscreenGuestDraw work)
        {
            ...
            try
            {
                // Finish prior RT/compute submits before resolving sampled textures
                // (upsample 054127 must be idle before composite samples it).
                EnsureGuestSubmissionCapacity();
                DrainInFlightGuestSubmissions();
                ...
                // Upsample→composite: prior submit should leave sampled RTs in
                // ShaderReadOnly, but re-barrier here so sample-time layout cannot
                // stay ColorAttachmentOptimal across queue batches.
                RecordSampledGuestImagesForRead(
                    resources,
                    target,
                    PipelineStageFlags.FragmentShaderBit);

                var targetHasPriorContents = target.Initialized || target.InitialUploadPending;
                var targetOldLayout = targetHasPriorContents
                    ? (target.Layout == ImageLayout.Undefined
                        ? ImageLayout.ShaderReadOnlyOptimal
                        : target.Layout)
                ...
                _vk.CmdPipelineBarrier(
                    ...
                    1,
                    &toColorAttachment);
                target.Layout = ImageLayout.ColorAttachmentOptimal;
                ...
                MarkSampledImagesInitialized(resources);
                MarkStorageImagesInitialized(resources, traceContents: false);
                NoteHdrPresentCandidate(target, format);
        ...
        private static uint GetGuestTextureFormat(Format format) =>
            format switch
            {
                ...
                Format.R32G32B32A32Sint => 76,
                Format.R32G32B32A32Sfloat => 77,
                _ => 0,
            };
        ...
        private void Render(double _)
        {
            ...
            var tracePresentedGuestAfterPresent = false;
            if (presentedGuestImage is not null)
            {
                    ...
                    _directPresentationCount is 1 or 30 or 120)
                {
                    ...
                    // Defer guest readback until after present — a transfer round-trip
                    // on the presentation command buffer before the sample draw has
                    // produced black ImageSample results for A2R10 flip targets.
                    tracePresentedGuestAfterPresent = true;
                }
            }
                             ...
                             ExposedPresentPixelsHaveColor(exposedPixels))
                    {
                        ...
                        if (ShouldTracePresentedGuestImageContentsForDiagnostics())
                        {
                            Console.Error.WriteLine(
                             ...
                             _directPresentationCount is 1 or 30 or 120)
                    {
                        // Flip target still clear — blit until composite has luminance.
                        Console.Error.WriteLine(
            ...
            {
                Console.Error.WriteLine(
                    $"[LOADER][TRACE] vk.present_upload bytes={pixels.Length} " +
                    $"cpu_exposure={cpuExposurePresent} " +
                    $"p0={pixels[2]:X2}{pixels[1]:X2}{pixels[0]:X2}{pixels[3]:X2}");
                RecordUpload(imageIndex);
                if (cpuExposurePresent &&
                    ShouldTracePresentedGuestImageContentsForDiagnostics() &&
                    !_tracedPresentedSwapchain)
                {
                    _tracedPresentedSwapchain = true;
                    RecordSwapchainReadbackForTrace(imageIndex);
                }
            ...
            else if (presentedGuestImage is not null)
            {
                ...
                {
                    Console.Error.WriteLine(
                        $"[LOADER][TRACE] vk.present_path=blit " +
                        $"src=0x{presentedGuestImage.Address:X16}");
                }
            ...
            else if (tracePresentedGuestAfterPresent)
            {
                CompletePendingPresentation(wait: true);
            }

            if (tracePresentedGuestAfterPresent && presentedGuestImage is not null)
            {
            ...
            try
            {
                ...
                var toTransfer = new ImageMemoryBarrier
                {
                    ...
                    SrcAccessMask = AccessFlags.ShaderReadBit |
                                    ...
                                    AccessFlags.TransferWriteBit,
                    DstAccessMask = AccessFlags.TransferReadBit,
                    OldLayout = image.Layout == ImageLayout.Undefined
                        ? ImageLayout.ShaderReadOnlyOptimal
                        : image.Layout,
                    NewLayout = ImageLayout.TransferSrcOptimal,
                    SrcQueueFamilyIndex = Vk.QueueFamilyIgnored,
            ...
            PipelineStageFlags shaderStage)
        {
            foreach (var texture in resources.Textures)
            {
                ...
                {
                    uploadedGuestImage.Layout = ImageLayout.ShaderReadOnlyOptimal;
                }
            }
        }
            ...
            PipelineStageFlags shaderStage)
        {
            ...
            var sourceToTransfer = new ImageMemoryBarrier
            {
                ...
                SrcAccessMask = AccessFlags.ShaderReadBit |
                                ...
                                AccessFlags.TransferWriteBit,
                DstAccessMask = AccessFlags.TransferReadBit,
                OldLayout = sourceImage.Layout == ImageLayout.Undefined
                    ? ImageLayout.ShaderReadOnlyOptimal
                    : sourceImage.Layout,
                NewLayout = ImageLayout.TransferSrcOptimal,
                SrcQueueFamilyIndex = Vk.QueueFamilyIgnored,
            ...
            _vk.CmdPipelineBarrier(
                ...
                2,
                shaderReadBarriers);
            sourceImage.Layout = ImageLayout.ShaderReadOnlyOptimal;
        }

        // Ensure published guest images bound as sampled textures are in
        // ShaderReadOnlyOptimal before the fragment shader samples them.
        private void RecordSampledGuestImagesForRead(
            TranslatedDrawResources resources,
            GuestImageResource? renderTarget,
            PipelineStageFlags shaderStage)
        {
            ...
            foreach (var texture in resources.Textures)
            {
                ...
                var oldLayout = guestImage.Layout == ImageLayout.Undefined
                    ? ImageLayout.ShaderReadOnlyOptimal
                    : guestImage.Layout;
                if (oldLayout == ImageLayout.ShaderReadOnlyOptimal)
                {
                    // Still insert a memory dependency so ColorAttachment writes from
                    // a prior queue submit are visible to this fragment sample.
                    var visibility = new ImageMemoryBarrier
                    {
                        SType = StructureType.ImageMemoryBarrier,
                        SrcAccessMask = AccessFlags.ColorAttachmentWriteBit |
                                        AccessFlags.ShaderWriteBit |
                ...
                _vk.CmdPipelineBarrier(
                    ...
                    1,
                    &barrier);
                guestImage.Layout = ImageLayout.ShaderReadOnlyOptimal;
            }
        }
            ...
            PipelineStageFlags shaderStage)
        {
            ...
            foreach (var texture in resources.Textures)
            {
                ...
                _vk.CmdPipelineBarrier(
                    ...
                    1,
                    &barrier);
                guestImage.Layout = ImageLayout.General;
            }
        }
        ...
        private void RecordSwapchainReadbackForTrace(uint imageIndex)
        {
